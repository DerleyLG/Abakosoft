...
const AbonoDrawer = ({ credito, onClose, onSaved }) => {
    ...
    const fetchMetodos = async () => {
      try {
        ...
        const filtered = (res.data || []).filter(
          (m) => (m.nombre || "").toLowerCase() !== "credito",
        );
        setMetodos(filtered);
      } catch (e) {
        // Silenciar error
  ...
  useEffect(() => {
    ...
    const fetchResumen = async () => {
      try {
        ...
        const resumenRes = await api
          .get(`/creditos/${credito.id_venta_credito}/resumen`)
          .catch(() => null);
  ...
  const handleSubmit = async () => {
    ...
    if (val > (credito.saldo_pendiente || 0))
      return toast.error("El monto no puede ser mayor al saldo pendiente");
    if (!metodo) return toast.error("Selecciona un método de pago");

    try {
      setGuardando(true);
      await api.post(
        `/creditos/${credito.id_venta_credito}/abonos`,
        {
          monto: val,
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-semibold">
            Registrar Abono - #{credito.id_orden_venta}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="font-mono text-lg">
              ${formatCOP(credito.saldo_pendiente || 0)}
            </div>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                className={`px-4 py-2 rounded ${guardando ? "bg-slate-300" : "bg-slate-600 text-white"} hover:brightness-110 cursor-pointer`}
              >
            ...
            <div className="mt-4">
              ...
              {resumen?.resumen ? (
                <div className="mt-2 text-sm">
                  <div>
                    Monto Total:{" "}
                    <strong>${formatCOP(resumen.resumen.monto_total)}</strong>
                  </div>
                  <div>
                    Total Abonado:{" "}
                    <strong>${formatCOP(resumen.resumen.total_abonado)}</strong>
                  </div>
                  <div>
                    Saldo Pendiente:{" "}
                    <strong>
                      ${formatCOP(resumen.resumen.saldo_pendiente)}
                    </strong>
                  </div>
                  <div>
                    Estado: <strong>{resumen.resumen.estado}</strong>
                  </div>
                </div>
              ) : (
                <div className="text-sm text-slate-500">
