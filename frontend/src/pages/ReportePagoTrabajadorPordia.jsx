...
const ReportePagoTrabajadorPorDia = () => {
  ...
  const { desde, hasta } = useMemo(() => {
    ...
    return {
      desde: formatLocalYMD(firstDayOfMonth),
      hasta: formatLocalYMD(lastDayOfMonth),
    };
  ...
  const endpoint = useMemo(() => {
    ...
    if (seleccion?.value)
      params.append("id_trabajador", String(seleccion.value));
    if (selectedDate) {
      const ymd = formatLocalYMD(selectedDate);
  ...
  const columnas = [
    ...
    },
    {
      ...
      accessor: (row) => {
        // devolver número solo si la fila es un anticipo; si no, devolver null para mostrar '--'
        if (row && row.tipo === "anticipo")
          return Number(row.total_anticipado) || 0;
        return null;
      },
      isCurrency: true,
    },
    ...
    <div className="p-6">
        ...
        <div className="w-full md:max-w-2xl">
          ...
          <div className="mt-2 text-xs text-slate-500">
            {selectedDate
              ...
              : (() => {
                  const mesLabel = new Intl.DateTimeFormat("es-CO", {
                    month: "long",
                    year: "numeric",
                  }).format(firstDayOfMonth);
                  return `Mostrando pagos de ${mesLabel}.`;
                })()}{" "}
            Si eliges un trabajador sin día, verás todos sus pagos. Si no eliges
            trabajador ni día, se muestra el mes actual.
          </div>
        </div>
      </div>
      ...
      <div className="mb-4">
        <div className="rounded-xl p-5 bg-slate-50 border border-slate-200 text-slate-800 shadow-sm flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-2 text-sm">
            <div>
              <span className="font-medium text-slate-600">Bruto:</span>{" "}
              <span className="font-semibold text-slate-900">
                {formatCOP(totales.bruto)}
              </span>
            </div>
            <div>
              <span className="font-medium text-slate-600">Descuentos:</span>{" "}
              <span className="font-semibold text-slate-900">
                {formatCOP(totales.descuentos)}
              </span>
            </div>
            <div>
              <span className="font-medium text-slate-600">Anticipado:</span>{" "}
              <span className="font-semibold text-slate-900">
                {formatCOP(totales.anticipado)}
              </span>
            </div>
            <div>
      ...
      <ReporteBase
        ...
        exportSummary={[
          ...
          { label: "Anticipado", value: totales.anticipado, isCurrency: true },
        ]}
        onDataChange={(rows) => {
          const bruto = rows.reduce(
            (acc, r) => acc + (Number(r.total_bruto) || 0),
            0,
          );
          const descuentos = rows.reduce(
            (acc, r) => acc + (Number(r.total_descuentos) || 0),
            0,
          );
          const neto = rows.reduce(
            (acc, r) => acc + (Number(r.total_neto) || 0),
            0,
          );
          const anticipado = rows.reduce(
            (acc, r) => acc + (Number(r.total_anticipado) || 0),
            0,
          );
          setTotales({ bruto, descuentos, neto, anticipado });
        }}
      />
    </div>
  );
