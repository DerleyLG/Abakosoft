...
module.exports = {
    ...
    try {
      if (
        ...
        return res.status(400).json({
          error:
            "Faltan campos obligatorios: id_articulo, cantidad, tipo_movimiento, origen.",
        });
      }
      ...
      if (
        ...
        cantidadNumerica < 0 ||
        (cantidadNumerica === 0 &&
          origen !== InventarioModel.TIPOS_ORIGEN_MOVIMIENTO.INICIAL)
      ) {
        return res.status(400).json({
          error:
            "La cantidad debe ser un número positivo, o 0 si es un ingreso inicial.",
        });
      }

      const [articuloExiste] = await db.query(
        "SELECT 1 FROM articulos WHERE id_articulo = ?",
        [id_articulo],
      );
      if (articuloExiste.length === 0) {
        return res.status(404).json({
          message: "Artículo no encontrado en la base de datos de artículos.",
        });
      }

      const inventarioActual =
        await InventarioModel.obtenerInventarioPorArticulo(id_articulo);
      if (
        inventarioActual &&
        origen === InventarioModel.TIPOS_ORIGEN_MOVIMIENTO.INICIAL
      ) {
        return res.status(400).json({
          error: `El artículo ${id_articulo} ya existe en el inventario. No se puede realizar un ingreso inicial duplicado.`,
        });
      }
      ...
      console.error(
        "Error al registrar movimiento y actualizar inventario:",
        error,
      );

      res.status(500).json({
        error: error.message || "Error interno al registrar el movimiento.",
      });
    }
  },
  ...
  getById: async (req, res) => {
    ...
    try {
      ...
      if (!articulo) {
        return res
          .status(404)
          .json({ mensaje: "Artículo no encontrado en inventario" });
      }
  ...
  actualizarInventario: async (req, res) => {
    ...
    try {
      ...
      if (nuevoStockTotal == null || nuevoStockMinimo == null) {
        return res
          .status(400)
          .json({ error: "Faltan campos requeridos: stock o stock_minimo." });
      }
      ...
      if (
        cantidadMovida === 0 &&
        Number(nuevoStockMinimo) !== inventarioActual.stock_minimo
      ) {
        await db.query(
        ...
        return res.json({
          message:
            "Stock mínimo actualizado correctamente (sin movimiento de stock).",
        });
      }
      ...
      await InventarioModel.processInventoryMovement({
        ...
        tipo_origen_movimiento:
          InventarioModel.TIPOS_ORIGEN_MOVIMIENTO.AJUSTE_MANUAL,
        observaciones: `Ajuste manual de stock. Stock anterior: ${
          inventarioActual.stock
        }, Nuevo stock: ${Number(nuevoStockTotal)}. Stock mínimo: ${Number(
          nuevoStockMinimo,
        )}`,
      ...
      res.json({
        message: "Inventario y stock mínimo actualizados correctamente",
      });
    } catch (error) {
      console.error("Error al actualizar inventario:", error);
      res
        .status(500)
        .json({ error: error.message || "Error al actualizar inventario." });
    }
  },
  ...
  eliminarArticulo: async (req, res) => {
    ...
    try {
      ...
      if (
        inventarioActual &&
        (inventarioActual.stock > 0 || inventarioActual.stock_fabricado > 0)
      ) {
        return res.status(400).json({
          message:
            "No se puede eliminar un artículo del inventario si tiene stock disponible o fabricado.",
        });
      }
      ...
      res.status(500).json({
        mensaje: error.message || "Error interno al eliminar el artículo",
      });
    }
  },
  ...
  inicializarArticuloEnInventario: async (req, res) => {
    ...
    try {
      ...
      if (!id_articulo) {
        return res.status(400).json({
          message: "ID de artículo es obligatorio para la inicialización.",
        });
      }

      // Validar que el stock inicial sea un número válido
      const stockInicialNum = parseFloat(stock_inicial);
      ...
      if (!articuloInfo) {
        ...
        connection.release();
        return res.status(404).json({
          message: `Artículo con ID ${id_articulo} no encontrado en el catálogo de artículos.`,
        });
      }
      ...
      await InventarioModel.processInventoryMovement(
        {
          id_articulo: Number(id_articulo),
          cantidad_movida: stockInicialNum,
          tipo_movimiento: InventarioModel.TIPOS_MOVIMIENTO.ENTRADA,
          tipo_origen_movimiento:
            InventarioModel.TIPOS_ORIGEN_MOVIMIENTO.INICIAL,
          observaciones: `Inicialización de artículo en inventario con stock inicial de ${stockInicialNum}.`,
          referencia_documento_id: null,
          referencia_documento_tipo: "inicializacion_manual",
          stock_minimo_inicial: stock_minimo,
        },
        connection,
      );
      ...
      res.status(200).json({
        success: true,
        message: `Artículo "${articuloInfo.descripcion}" inicializado en inventario con stock ${stockInicialNum} correctamente.`,
        data: {
          id_articulo,
          descripcion: articuloInfo.descripcion,
          stock_disponible: stockInicialNum,
        },
      });
    } catch (error) {
      if (connection) {
