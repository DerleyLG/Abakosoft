...
const etapasProduccionController = {
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "El orden de la etapa es obligatorio y debe ser un número entero positivo.",
        });
      }

      // Si el orden está ocupado, desplazar las etapas existentes hacia adelante
      const ordenExistente = await etapasModel.existsByOrden(orden);
      if (ordenExistente) {
        await etapasModel.shiftOrdenUp(Number(orden));
      }

      const { cargo } = req.body;
      const id = await etapasModel.create({
        nombre,
        descripcion,
        orden: Number(orden),
        cargo: cargo || null,
  ...
  update: async (req, res) => {
    try {
      const { id } = req.params;
      const { nombre, descripcion, orden, cargo } = req.body;
      ...
      if (Number(orden) !== etapa.orden) {
        // Solo si el orden ha cambiado
        const ordenExistente = await etapasModel.existsByOrden(orden);
        if (ordenExistente) {
          return res
            .status(400)
            .json({ error: `Ya existe una etapa con el orden ${orden}.` });
        }
      }

      await etapasModel.update(id, {
        nombre,
        descripcion,
        orden: Number(orden),
        cargo: cargo || null,
  ...
  delete: async (req, res) => {
    ...
    try {
      const etapa = await etapasModel.getById(req.params.id);
      if (!etapa) return res.status(404).json({ error: "Etapa no encontrada" });
      const ordenEliminado = etapa.orden;
      await etapasModel.delete(req.params.id);
      await etapasModel.shiftOrdenDown(ordenEliminado);
