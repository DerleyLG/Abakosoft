...
module.exports = {
  ...
  create: async (
    { id_pago, id_avance_etapa, cantidad, pago_unitario, es_descuento = 0 },
    connection = null,
  ) => {
    try {
      const idAvance = id_avance_etapa !== undefined ? id_avance_etapa : null;
      const dbConn = connection || db;
      // subtotal es columna generada; no debemos especificarlo en el INSERT
      const [result] = await dbConn.query(
        `INSERT INTO detalle_pago_trabajador (id_pago, id_avance_etapa, cantidad, pago_unitario, es_descuento)
         VALUES (?, ?, ?, ?, ?)`,
        [id_pago, idAvance, cantidad, pago_unitario, es_descuento ? 1 : 0],
      );
  ...
  delete: async (id) => {
    await db.query(
      "DELETE FROM detalle_pago_trabajador WHERE id_detalle_pago = ?",
      [id],
    );
  },
